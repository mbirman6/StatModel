...
%     check_ffm(y)=any(isnan(comp_ffm-ones(N,sz)));
    ...
    %% Find zero of dL/dmu=f(b_hh)
    % mu0=6;
    % MU_z=NaN(N,1); B1_z=NaN(N,1); B2_z=NaN(N,1); comp_fz=NaN(N,1); Lmin_z=NaN(N,1);
    % for i=1:N
    %     n1=In1(i); m1=Im1(i); n2=In2(i); m2=Im2(i); %s1=Is1(i); s2=Is2(i);
    %     X=fzero(@(P)s1+s2-(4*m1*s1)/(m1+n1+2*P*s1+(4*P^2*s1^2-4*P*m1*s1+4*P*n1*s1+m1^2+2*m1*n1+ ...
    %                 n1^2)^(1/2))-(4*m2*s2)/(m2+n2+2*P*s2+(4*P^2*s2^2-4*P*m2*s2+4*P*n2*s2+m2^2+2*m2*n2+ ...
    %                 n2^2)^(1/2)),mu0);
    % %     X=fzero(@(P)m1+m2+n1+n2-sqrt(m1^2+2*m1*(n1-2*P*s1)+(n1+2*P*s1)^2) ...
    % %                        -sqrt(m2^2+2*m2*(n2-2*P*s2)+(n2+2*P*s2)^2),mu0);
    %            
    %     MU_z(i)=X;
    %     B1_z(i)=m1/4+n1/4-(X*s1)/2+(4*X^2*s1^2-4*X*m1*s1+4*X*n1*s1+m1^2+2*m1*n1+n1^2)^(1/2)/4;
    %     B2_z(i)=m2/4+n2/4-(X*s2)/2+(4*X^2*s2^2-4*X*m2*s2+4*X*n2*s2+m2^2+2*m2*n2+n2^2)^(1/2)/4;
    %     
    % %     if((abs(MU_f(i)-MU_z(i))<=0.001)&&(abs(B1_f(i)-B1_z(i))<=0.001)&&(abs(B2_f(i)-B2_z(i))<=0.001)), comp_fz(i)=1;
    % %     else comp_fz(i)=0;
    % %     end
    % end

    % check_fz=any(comp_fz-ones(N,1));

    % Lmin_z=2*(B1_z+B2_z)+MU_z.*(Is1+Is2)-log(B1_z.^In1.*(B1_z+MU_z.*Is1).^Im1.*B2_z.^In2.*(B2_z+MU_z.*Is2).^Im2 ...
    % %    ./(factorial(In1).*factorial(Im1).*factorial(In2).*factorial(Im2)));
    % Lmin_z=2*(B1_z+B2_z)+MU_z.*(s1+s2)-log(B1_z.^In1.*(B1_z+MU_z.*s1).^Im1.*B2_z.^In2.*(B2_z+MU_z.*s2).^Im2 ...
    %    ./(factorial(In1).*factorial(Im1).*factorial(In2).*factorial(Im2)));
...
%%
clear('m1','m2','n1','n2','i','mm1','mm2','nn1','nn2','X','fval','mu0','lb','ub','options','A','x0');%,'ddLmu','ddLb1','ddLb2');
